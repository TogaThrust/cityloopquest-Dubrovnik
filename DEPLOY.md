# Déploiement — CityLoop Quest Dubrovnik

Guide généré par **CLQ App Factory** pour **Dubrovnik** (`dubrovnik`).  
Dossier de sortie : `CLQ-DUBROVNIK/clq-dubrovnik` — URL Netlify cible : **https://clq-dubrovnik.netlify.app**
...
|--------|-------------------------|
| Slug backend / DB | `dubrovnik` |
| Package npm | `clq-dubrovnik` |
| Site Netlify | `clq-dubrovnik` |
| Site ID Netlify | `` |
| Dépôt CI (GitHub → Netlify) | `TogaThrust/cityloopquest-Dubrovnik` |
| URL publique | https://clq-dubrovnik.netlify.app |
| API (Render) | https://cityloopquest-api.onrender.com |
| Région | Comitat de Dubrovnik-Neretva, Croatie |
...
2. Charger ou compléter le JSON ville (**Dubrovnik**).
3. Lancer **Générer l’app** → sortie dans `CLQ-DUBROVNIK/clq-dubrovnik`.
4. Vérifier dans la Factory : audios, images POI, quiz, traductions sans erreur bloquante.
5. Tester en local :
   ```bash
   cd "CLQ-DUBROVNIK/clq-dubrovnik"
   npm install
   npm run dev
   ```
...
| Rôle | Exemple | Usage |
|------|---------|--------|
| **Dev / Factory** | `clq-dubrovnik` (privé) | Travail quotidien, génération Factory |
| **CI Netlify** | `TogaThrust/cityloopquest-Dubrovnik` (public) | Push déclenche le build Netlify |

1. Pousser le code vers le dépôt dev (`origin`).
2. Ajouter le remote CI si besoin :
   ```bash
   git remote add cityloopquest https://github.com/TogaThrust/cityloopquest-Dubrovnik.git
   ```
...
> **Lille / villes déjà liées dans Netlify :** le push sur `TogaThrust/cityloopquest-Dubrovnik` suffit souvent ; le workflow GitHub Actions sert de secours si l’app GitHub Netlify n’a pas accès au dépôt.
...
À définir dans **Site settings → Environment variables** (pas la liste complète d’une autre ville — uniquement ce dont **Dubrovnik** a besoin) :
...
| `SITE_URL` | https://clq-dubrovnik.netlify.app |
...
**Site ID :** `` — admin : https://app.netlify.com/projects/clq-dubrovnik
...
- `https://clq-dubrovnik.netlify.app/*`
...
> **Noms de variables :** pour un slug multi-mots (`dubrovnik`), utiliser des **underscores** dans le préfixe env (`DUBROVNIK` = tirets du slug remplacés par `_`, ex. `caravaca-de-la-cruz` → `CARAVACA_DE_LA_CRUZ`). Ne pas utiliser de tirets ni un slug raccourci.

| Variable | Exemple / valeur |
|----------|------------------|
| `CLIENT_BASE_URL_DUBROVNIK` | `https://clq-dubrovnik.netlify.app` |
| `STRIPE_DUBROVNIK_FULL_7D_PRICE_ID` | `price_xxx` (depuis Stripe) |
| `STRIPE_DUBROVNIK_LITE_7D_PRICE_ID` | si applicable |
| `STRIPE_DUBROVNIK_UPGRADE_7D_PRICE_ID` | si applicable |

Mettre à jour **CORS / ALLOWED_ORIGINS** pour inclure `https://clq-dubrovnik.netlify.app`.

Dans le code API (si nouvelle ville) :

- Ajouter `dubrovnik` aux villes supportées (checkout, claim-session, activate-code).
- Mapper les Price IDs et `CLIENT_BASE_URL_DUBROVNIK` dans la config (ex. `STRIPE_PRICE_IDS`, `FRONTEND_URLS`).
...
1. Créer les **produits / prix** 7 jours pour **Dubrovnik** (FULL, LITE, UPGRADE si utilisés).
...
4. Ne pas configurer d’URL success/cancel figées dans Stripe : elles sont construites côté API avec `CLIENT_BASE_URL_DUBROVNIK`.
...
1. Table **`cities`** : entrée `slug = 'dubrovnik'`, nom `Dubrovnik`, métadonnées région.
2. Table **`stripe_products_map`** (ou équivalent) : lier les Price ID Stripe à `dubrovnik`.
3. Table **`licenses`** : codes d’activation `xxx-xxx-xxx` rattachés à `city = 'dubrovnik'`.
...
Le fichier `js/license-city-guard.js` associe le hostname au slug `dubrovnik`.  
Après génération Factory, vérifier que la ligne contient bien :

```js
if (host.includes('dubrovnik')) return 'dubrovnik';
```

**Piège :** `localStorage` est partagé entre tous les sites `*.netlify.app` sur un même navigateur. Une licence **Murcia** ne doit pas débloquer **Dubrovnik** — le guard compare `clq_city` / JWT au slug du site.
...
- [ ] `mail.html` mentionne **Dubrovnik**.
- [ ] PWA : `manifest.json`, icônes `cityLoopQuest_Dubrovnik_*.png`.
- [ ] Popup fin de parcours + invitation selfie.
- [ ] CORS : appels API depuis https://clq-dubrovnik.netlify.app OK.
...
| Install PWA absente (barre d’adresse) | `index.html` doit charger `js/clq-sw-register.js` ; icônes `cityLoopQuest_Dubrovnik_{192,512}.png` présentes ; HTTPS ou localhost. |
...
3. `npm run build && npm run deploy` dans `CLQ-DUBROVNIK/clq-dubrovnik`.

---

*Document généré pour CityLoop Quest Dubrovnik — slug `dubrovnik`.*
