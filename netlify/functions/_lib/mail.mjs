...
function siteUrl() {
  return (process.env.SITE_URL || "https://clq-dubrovnik.netlify.app").replace(/\/+$/, "");
}
...
export async function mailStaffPoiProposal({
  ...
  const html = `
    <h2>Nouvelle proposition de POI — CLQ Dubrovnik</h2>
  ...
  await tx.sendMail({
    ...
    to: staffEmail(),
    subject: `[CLQ Dubrovnik] POI a valider : ${name}`,
    html,
    attachments,
  });
  return true;
...
export async function mailSubmitterResult({ to, name, approved }) {
  ...
  const subject = approved
    ? `[CLQ Dubrovnik] Votre point d'interet a ete publie`
    : `[CLQ Dubrovnik] Proposition de POI non retenue`;
  const body = approved
    ? `Bonjour,\n\nVotre proposition « ${name} » a ete validee par notre equipe et est maintenant visible dans l'application CLQ Dubrovnik.\n\nMerci pour votre contribution !`
