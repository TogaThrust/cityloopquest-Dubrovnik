...
function page(title, message, ok) {
  ...
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${title}</title></head><body style="font-family:Arial,sans-serif;max-width:520px;margin:40px auto;padding:20px;"><h1 style="color:${color}">${title}</h1><p>${message}</p><p><a href="https://clq-dubrovnik.netlify.app/poi-experiment.html">Retour a l'app POI</a></p></body></html>`;
}
...
export async function handler(event) {
  ...
  try {
    ...
    const hint = /blobs|siteID|token/i.test(msg)
      ? " Activez Netlify Blobs (Storage) pour le site clq-dubrovnik, redeployez, puis renvoyez une proposition si besoin."
      : "";
