...
function clqStorageKey(suffix) {
  const prefix = (window.CLQ_CITY && window.CLQ_CITY.storagePrefix) || 'dubrovnik';
...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('dubrovnik_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('dubrovnik_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('dubrovnik_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioCompletedForIndex(idx) {
    ...
    localStorage.setItem(
        'dubrovnik_audio_completed_indices',
        JSON.stringify([...set])
    );
}
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('dubrovnik_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('dubrovnik_audio_clicked_index', String(idx));
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("dubrovnik_completedQuizQuestions");
...
const MONS_LAST_KNOWN_POSITION_KEY = 'dubrovnik_lastKnownPosition';
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('dubrovnik_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('dubrovnik_audio_heard_indices');
  localStorage.removeItem('dubrovnik_audio_clicked_index');
  localStorage.removeItem('dubrovnik_audio_completed_indices');
  localStorage.removeItem('dubrovnik_quizEnabled');
  localStorage.removeItem('dubrovnik_completedQuizQuestions');
  quizEnabled = false;
  quizResumeAwaitingAudio = false;
  quizPendingAdvanceCallback = null;
...
/** Variante fichier image Dubrovnik : espaces   underscores, casse conservée. */
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('dubrovnik_currentIndex', String(currentIndex));
    localStorage.setItem('dubrovnik_score', String(score));
    try {
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('dubrovnik_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('dubrovnik_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("dubrovnik_currentIndex", currentIndex);
        localStorage.setItem("dubrovnik_score", score);
...
function clearQuizCompletionFromIndex(fromIdx) {
  ...
  localStorage.setItem('dubrovnik_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
}
...
function updateQuizFooterButton() {
  ...
  const show = !guidanceMode && localStorage.getItem('dubrovnik_quizEnabled') === 'false';
...
function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('dubrovnik_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('dubrovnik_quizStopped');
  updateQuizFooterButton();
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('dubrovnik_quizStopped', 'true');
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("dubrovnik_quizEnabled") !== null) {
    callback(localStorage.getItem("dubrovnik_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("dubrovnik_quizEnabled", "true");
    quizEnabled = true;
    completedQuizQuestions = {};
    localStorage.removeItem('dubrovnik_completedQuizQuestions');
    localStorage.removeItem('dubrovnik_quizStopped');
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("dubrovnik_quizEnabled", "false");
    quizEnabled = false;
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
...
function initializeMainLogic() {
    ...
    const savedCurrentIndex = localStorage.getItem("dubrovnik_currentIndex");
    const savedScore = localStorage.getItem("dubrovnik_score");
    const savedQuizEnabled = freshCircuitStart
        ? null
        : localStorage.getItem("dubrovnik_quizEnabled");
    const savedCompletedQuizQuestions = freshCircuitStart
        ? null
        : localStorage.getItem("dubrovnik_completedQuizQuestions");
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('dubrovnik_visitedPoints'), []);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (localStorage.getItem("dubrovnik_quizEnabled") === null) {
            ...
            quizEnabled = localStorage.getItem("dubrovnik_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
            } else {
                advanceAfterQuiz();
